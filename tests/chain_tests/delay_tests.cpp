...
BOOST_AUTO_TEST_CASE( mindelay_test ) { try {
   ...
   const auto& acnt = chain.control->get_database().get<account_object,by_name>(N(enumivo.coin));
   const auto abi = acnt.get_abi();
   chain::contracts::abi_serializer abis(abi);
   const auto a = chain.control->get_database().get<account_object,by_name>(N(enumivo.coin)).get_abi();
