...
namespace enumivo {
   ...
   void exchange::deposit( account_name from, extended_asset quantity ) {
      enu_assert( quantity.is_valid(), "invalid quantity" );
   ...
   void exchange::withdraw( account_name from, extended_asset quantity ) {
      require_auth( from );
      enu_assert( quantity.is_valid(), "invalid quantity" );
      enu_assert( quantity.amount >= 0, "cannot withdraw negative balance" ); // Redundant? inline_transfer will fail if quantity is not positive.
   ...
   void exchange::on( const trade& t ) {
      ...
      enu_assert( t.sell.is_valid(), "invalid sell amount" );
      enu_assert( t.sell.amount > 0, "sell amount must be positive" );
      enu_assert( t.min_receive.is_valid(), "invalid min receive amount" );
      enu_assert( t.min_receive.amount >= 0, "min receive amount cannot be negative" );

      auto receive_symbol = t.min_receive.get_extended_symbol();
      enu_assert( t.sell.get_extended_symbol() != receive_symbol, "invalid conversion" );
      ...
      if( t.min_receive.amount != 0 ) {
         enu_assert( t.min_receive.amount <= output.amount, "unable to fill" );
      }
   ...
   void exchange::on( const upmargin& b ) {
      ...
      enu_assert( b.delta_borrow.is_valid(), "invalid borrow delta" );
      enu_assert( b.delta_collateral.is_valid(), "invalid collateral delta" );

      market_state market( _this_contract, b.market, _accounts );

      enu_assert( b.delta_borrow.amount != 0 || b.delta_collateral.amount != 0, "no effect" );
      enu_assert( b.delta_borrow.get_extended_symbol() != b.delta_collateral.get_extended_symbol(), "invalid args" );
      enu_assert( market.exstate.base.balance.get_extended_symbol() == b.delta_borrow.get_extended_symbol() ||
                    market.exstate.quote.balance.get_extended_symbol() == b.delta_borrow.get_extended_symbol(),
                    "invalid asset for market" );
      enu_assert( market.exstate.base.balance.get_extended_symbol() == b.delta_collateral.get_extended_symbol() ||
   ...
   void exchange::on( const covermargin& c ) {
      ...
      enu_assert( c.cover_amount.is_valid(), "invalid cover amount" );
      enu_assert( c.cover_amount.amount > 0, "cover amount must be positive" );
   ...
   void exchange::createx( account_name    creator,
      ...
      enu_assert( initial_supply.is_valid(), "invalid initial supply" );
      enu_assert( initial_supply.amount > 0, "initial supply must be positive" );
      enu_assert( base_deposit.is_valid(), "invalid base deposit" );
      enu_assert( base_deposit.amount > 0, "base deposit must be positive" );
      enu_assert( quote_deposit.is_valid(), "invalid quote deposit" );
      enu_assert( quote_deposit.amount > 0, "quote deposit must be positive" );
      enu_assert( base_deposit.get_extended_symbol() != quote_deposit.get_extended_symbol(),
      ...
      enu_assert( existing == exstates.end(), "market already exists" );
   ...
   void exchange::lend( account_name lender, symbol_type market, extended_asset quantity ) {
      require_auth( lender );
      enu_assert( quantity.is_valid(), "invalid quantity" );
      enu_assert( quantity.amount > 0, "must lend a positive amount" );
   ...
   void exchange::unlend( account_name lender, symbol_type market, double interest_shares, extended_symbol interest_symbol ) {
      require_auth( lender );
      enu_assert( interest_shares > 0, "must unlend a positive amount" );
   ...
   void exchange::on( const currency::transfer& t, account_name code ) {
      ...
      if( t.to == _this_contract ) {
         ...
         enu_assert( a.is_valid(), "invalid quantity in transfer" );
         enu_assert( a.amount != 0, "zero quantity is disallowed in transfer");
         enu_assert( a.amount > 0 || t.memo == "withdraw", "withdrew tokens without withdraw in memo");
         enu_assert( a.amount < 0 || t.memo == "deposit", "received tokens without deposit in memo" );
