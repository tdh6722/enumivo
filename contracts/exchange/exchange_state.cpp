...
namespace enumivo {
   ...
   extended_asset exchange_state::convert_from_exchange( connector& c, extended_asset in ) {
      enu_assert( in.contract == supply.contract, "unexpected asset contract input" );
      enu_assert( in.symbol== supply.symbol, "unexpected asset symbol input" );
   ...
   extended_asset exchange_state::convert( extended_asset from, extended_symbol to ) {
      ...
      if( sell_symbol != ex_symbol ) {
         ...
         if( to == base_symbol ) {
            ...
            enu_assert( false, "invalid conversion" );
         }
      }
