...
void verify_mem(const void* const ptr, const uint32_t val, const uint32_t size)
{
   ...
   {
      enu_assert(static_cast<uint32_t>(static_cast<unsigned char>(char_ptr[i])) == val, "buf slot doesn't match");
   }
}
...
void test_memory::test_memory_allocs()
{
    ...
    enu_assert(ptr1 == nullptr, "should not have allocated a 0 char buf");

    // 20 chars - 20 + 4(header) which is divisible by 8
    ptr1 = (char*)malloc(20);
    enu_assert(ptr1 != nullptr, "should have allocated a 20 char buf");
    ...
    enu_assert(ptr1_realloc != nullptr, "should have returned a 30 char buf");
    enu_assert(ptr1_realloc == ptr1, "should have enlarged the 20 char buf");
    // existing memory layout -> |40|

    // 20 chars allocated
    char* ptr2 = (char*)malloc(20);
    enu_assert(ptr2 != nullptr, "should have allocated another 20 char buf");
    enu_assert(ptr1 + 36 < ptr2, "20 char buf should have been created after ptr1"); // test specific to implementation (can remove for refactor)
    verify_mem(ptr1, 0, 36);
    enu_assert(ptr1[36] != 0, "should not have empty bytes following since block allocated"); // test specific to implementation (can remove for refactor)
    ...
    enu_assert(ptr1_realloc != nullptr, "should have returned a 15 char buf");
    enu_assert(ptr1_realloc == ptr1, "should have shrunk the reallocated 30 char buf");
    verify_mem(ptr1, 0, 14); // test specific to implementation (can remove for refactor)
    enu_assert(ptr1[14] == 0x7e, "remaining 15 chars of buf should be untouched");
    // existing memory layout -> |24(shrunk)|16(freed)|24|

    //same size the buffer (verify corner case)
    // 20 chars allocated (still)
    ptr1_realloc = (char*)realloc(ptr1, 15);
    enu_assert(ptr1_realloc != nullptr, "should have returned a reallocated 15 char buf");
    enu_assert(ptr1_realloc == ptr1, "should have reallocated 15 char buf as the same buf");
    enu_assert(ptr1[14] == 0x7e, "remaining 15 chars of buf should be untouched for unchanged buf");
    ...
    enu_assert(ptr1_realloc != nullptr, "should have returned a 30 char buf");
    enu_assert(ptr1_realloc == ptr1, "should have increased the buf back to orig max"); //test specific to implementation (can remove for refactor)
    enu_assert(ptr1[14] == 0x7e, "remaining 15 chars of buf should be untouched for expanded buf");

    //increase buffer beyond (indicated) allocated space
    // 36 chars allocated (still)
    ptr1_realloc = (char*)realloc(ptr1, 36);
    enu_assert(ptr1_realloc != nullptr, "should have returned a 36 char buf");
    enu_assert(ptr1_realloc == ptr1, "should have increased char buf to actual size"); // test specific to implementation (can remove for refactor)
    ...
    enu_assert(ptr1_realloc != nullptr, "should have returned a 37 char buf");
    enu_assert(ptr1_realloc != ptr1, "should have had to create new 37 char buf from 36 char buf");
    enu_assert(ptr2 < ptr1_realloc, "should have been created after ptr2"); // test specific to implementation (can remove for refactor)
    enu_assert(ptr1_realloc[14] == 0x7e, "orig 36 char buf's content should be copied");
    enu_assert(ptr1_realloc[35] == 0x7f, "orig 36 char buf's content should be copied");

    //realloc with nullptr
    char* nullptr_realloc = (char*)realloc(nullptr, 50);
    enu_assert(nullptr_realloc != nullptr, "should have returned a 50 char buf and ignored nullptr");
    enu_assert(ptr1_realloc < nullptr_realloc, "should have created after ptr1_realloc"); // test specific to implementation (can remove for refactor)

    //realloc with invalid ptr
    char* invalid_ptr_realloc = (char*)realloc(nullptr_realloc + 4, 10);
    enu_assert(invalid_ptr_realloc != nullptr, "should have returned a 10 char buf and ignored invalid ptr");
    enu_assert(nullptr_realloc < invalid_ptr_realloc, "should have created invalid_ptr_realloc after nullptr_realloc"); // test specific to implementation (can remove for refactor)
}
...
void test_memory::test_memory_hunk()
{
   ...
   enu_assert(ptr1 != nullptr, "should have allocated a ~983K char buf");
}
...
void test_memory::test_memory_hunks()
{
   ...
   enu_assert(ptr1 != nullptr, "should have allocated a 7404 char buf");
   ...
   for (int i = 0; i < 96; ++i)
   {
      ...
      enu_assert(ptr2 != nullptr, "should have allocated a ~10K char buf");
      if (last_ptr != nullptr)
      {
         // - 15 rounds to -8
         enu_assert(last_ptr + 10 * 1024 - 8 == ptr2, "should allocate the very next ptr"); // test specific to implementation (can remove for refactor)
      }
   ...
   enu_assert(ptr3 != nullptr, "should have allocated a 772 char buf");
   enu_assert(ptr1 + 7408 == ptr3, "should allocate the very next ptr after ptr1 in initial heap"); // test specific to implementation (can remove for refactor)

   // use all but 8 chars
   char* ptr4 = (char*)malloc(764);
   enu_assert(ptr4 != nullptr, "should have allocated a 764 char buf");
   enu_assert(last_ptr + 10 * 1024 - 8 == ptr4, "should allocate the very next ptr after last_ptr at end of contiguous heap"); // test specific to implementation (can remove for refactor)

   // use up remaining 8 chars
   char* ptr5 = (char*)malloc(4);
   enu_assert(ptr5 != nullptr, "should have allocated a 4 char buf");
   enu_assert(ptr3 + 776 == ptr5, "should allocate the very next ptr after ptr3 in initial heap"); // test specific to implementation (can remove for refactor)

   // nothing left to allocate
   char* ptr6 = (char*)malloc(4);
   enu_assert(ptr6 == nullptr, "should not have allocated a char buf");
}
...
void test_memory::test_memory_hunks_disjoint()
{
   ...
   enu_assert(ptr1 != nullptr, "should have allocated a 8184 char buf");
   ...
   for (int i = 0; i < 14; ++i)
   {
      ...
      enu_assert(loop_ptr1[i] != nullptr, "should have allocated a 64K char buf");

      enu_assert(reinterpret_cast<int32_t>(sbrk(4)) != -1, "should be able to allocate 8 bytes");
   }
   ...
   enu_assert(ptr2 != nullptr, "should have allocated a 65412 char buf");
   ...
   for (int i = 0; i < 14; ++i)
   {
      ...
      enu_assert(loop_ptr2[i] != nullptr, "should have allocated a 12 char buf");
      enu_assert(loop_ptr1[i] + 64 * 1024 - 24 == loop_ptr2[i], "loop_ptr2[i] should be very next pointer after loop_ptr1[i]");
   }
   ...
   enu_assert(ptr3 != nullptr, "should have allocated a 4 char buf");
   enu_assert(loop_ptr2[13] + 16 == ptr3, "should allocate the very next ptr after loop_ptr2[13]"); // test specific to implementation (can remove for refactor)

   char* ptr4 = (char*)malloc(4);
   enu_assert(ptr4 != nullptr, "should have allocated a 4 char buf");
   enu_assert(ptr2 + 65416 == ptr4, "should allocate the very next ptr after ptr2 in last heap"); // test specific to implementation (can remove for refactor)

   char* ptr5 = (char*)malloc(4);
   enu_assert(ptr5 != nullptr, "should have allocated a 4 char buf");
   enu_assert(ptr1 + 8184 == ptr5, "should allocate the very next ptr after ptr1 in last heap"); // test specific to implementation (can remove for refactor)
   ...
   for (int i = 0; i < 13; ++i)
   {
      ...
      enu_assert(loop_ptr3[i] != nullptr, "should have allocated a 4 char buf");
      enu_assert(loop_ptr2[i] + 16 == loop_ptr3[i], "loop_ptr3[i] should be very next pointer after loop_ptr2[i]");
   }
   ...
   for (int i = 0; i < 64; ++i)
   {
      ...
      enu_assert(slot3_ptr[i] != nullptr, "should have allocated a 1020 char buf");
      if (i == 0)
         enu_assert(loop_ptr1[3] == slot3_ptr[0], "loop_ptr1[3] should be very next pointer after slot3_ptr[0]");
      else
         enu_assert(slot3_ptr[i - 1] + 1024 == slot3_ptr[i], "slot3_ptr[i] should be very next pointer after slot3_ptr[i-1]");
   }

   char* ptr7 = (char*)malloc(4);
   enu_assert(ptr7 == nullptr, "should not have allocated a char buf");
}   
...
void test_memory::test_memcmp()
{
   ...
   enu_assert(res1 == 0, "first data should be equal to second data");

   char buf3[] = "abcde";
   char buf4[] = "fghij";
   int32_t res2 = memcmp(buf3, buf4, 6);
   enu_assert(res2 < 0, "first data should be smaller than second data");

   char buf5[] = "fghij";
   char buf6[] = "abcde";
   int32_t res3 = memcmp(buf5, buf6, 6);
   enu_assert(res3 > 0, "first data should be larger than second data");
}
