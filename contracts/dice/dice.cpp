...
class dice : public enumivo::contract {
   public:
      ...
      void offerbet(const asset& bet, const account_name player, const checksum256& commitment) {
         ...
         enu_assert( amount.quantity > 0, "invalid bet" );
         enu_assert( !has_offer( commitment ), "offer with this commitment already exist" );
         require_auth( player );

         auto cur_player_itr = accounts.find( player );
         enu_assert(cur_player_itr != accounts.end(), "unknown account");
      ...
      void canceloffer( const checksum256& commitment ) {
         ...
         enu_assert( offer_itr != idx.end(), "offer does not exists" );
         enu_assert( offer_itr->gameid == 0, "unable to cancel offer" );
      ...
      void reveal( const checksum256& commitment, const checksum256& source ) {
         ...
         enu_assert(curr_revealer_offer != idx.end(), "offer not found");
         enu_assert(curr_revealer_offer->gameid > 0, "unable to reveal");
         ...
         enu_assert( is_zero(curr_reveal.reveal) == true, "player already revealed");
      ...
      void claimexpired( const uint64_t gameid ) {
         ...
         enu_assert(game_itr != games.end(), "game not found");
         enu_assert(game_itr->deadline != 0 && now() > game_itr->deadline, "game not expired");
         ...
         if( !is_zero(game_itr->player1.reveal) ) {
            enu_assert( is_zero(game_itr->player2.reveal), "game error");
            pay_and_clean(*game_itr, *player1_offer, *player2_offer);
         } else {
            enu_assert( is_zero(game_itr->player1.reveal), "game error");
      ...
      void withdraw( const account_name to, const asset& a ) {
         ...
         enu_assert(itr != accounts.end(), "unknown account");
