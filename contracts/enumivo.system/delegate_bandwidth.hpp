   ...
   class delegate_bandwidth : public voting<SystemAccount> {
      public:
         ...
         static void on( const delegatebw& del ) {
            enu_assert( del.stake_cpu_quantity.amount >= 0, "must stake a positive amount" );
            enu_assert( del.stake_net_quantity.amount >= 0, "must stake a positive amount" );
            enu_assert( del.stake_storage_quantity.amount >= 0, "must stake a positive amount" );

            system_token_type total_stake = del.stake_cpu_quantity + del.stake_net_quantity + del.stake_storage_quantity;
            enu_assert( total_stake.quantity > 0, "must stake a positive amount" );

            require_auth( del.from );

            //enu_assert( is_account( del.receiver ), "can only delegate resources to an existing account" );
            uint64_t storage_bytes = 0;
            if ( 0 < del.stake_storage_quantity.amount ) {
               ...
               enu_assert( 0 < storage_bytes, "stake is too small to increase storage even by 1 byte" );
         ...
         static void on( account_name receiver, const undelegatebw& del ) {
            enu_assert( del.unstake_cpu_quantity.amount >= 0, "must unstake a positive amount" );
            enu_assert( del.unstake_net_quantity.amount >= 0, "must unstake a positive amount" );
            ...
            enu_assert( dbw.net_weight >= del.unstake_net_quantity, "insufficient staked net bandwidth" );
            enu_assert( dbw.cpu_weight >= del.unstake_cpu_quantity, "insufficient staked cpu bandwidth" );
            enu_assert( dbw.storage_bytes >= del.unstake_storage_bytes, "insufficient staked storage" );
            ...
            enu_assert( total_refund.quantity > 0, "must unstake a positive amount" );
         ...
         static void on( const refund& r ) {
            ...
            enu_assert( req != refunds_tbl.end(), "refund request not found" );
            enu_assert( req->request_time + refund_delay <= now(), "refund is not available yet" );
