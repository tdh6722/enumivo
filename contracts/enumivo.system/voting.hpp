   ...
   class voting {
      public:
         ...
         static void on( const unregprod& unreg ) {
            ...
            enu_assert( prod != producers_tbl.end(), "producer not found" );
         ...
         static void increase_voting_power( account_name acnt, system_token_type amount ) {
            ...
            if ( voter->proxy ) {
               ...
               enu_assert( proxy != voters_tbl.end(), "selected proxy not found" ); //data corruption
            ...
            if ( producers ) {
               ...
               for( auto p : *producers ) {
                  ...
                  enu_assert( prod != producers_tbl.end(), "never existed producer" ); //data corruption
         ...
         static void decrease_voting_power( account_name acnt, system_token_type amount ) {
            ...
            voters_table voters_tbl( SystemAccount, SystemAccount );
            auto voter = voters_tbl.find( acnt );
            enu_assert( voter != voters_tbl.end(), "stake not found" );

            if ( 0 < amount.quantity ) {
               enu_assert( amount <= voter->staked, "cannot unstake more than total stake amount" );
         ...
         static void update_elected_producers(time cycle_time) {
            ...
            for ( auto it = idx.crbegin(); it != idx.crend() && n < 21 && 0 < it->total_votes; ++it ) {
               if ( it->active() ) {
                  ...
                  enu_assert( sizeof(schedule.producers.back().block_signing_key) == it->packed_key.size(), "size mismatch" );
         ...
         static void on( const voteproducer& vp ) {
            ...
            //validate input
            if ( vp.proxy ) {
               enu_assert( vp.producers.size() == 0, "cannot vote for producers and proxy at same time" );
               require_recipient( vp.proxy );
            } else {
               enu_assert( vp.producers.size() <= 30, "attempt to vote for too many producers" );
               for( size_t i = 1; i < vp.producers.size(); ++i ) {
                  enu_assert( vp.producers[i-1] < vp.producers[i], "producer votes must be unique and sorted" );
               }
            }
            ...
            enu_assert( voter != voters_tbl.end() && ( 0 < voter->staked.quantity || ( voter->is_proxy && 0 < voter->proxied_votes ) ), "no stake to vote" );
            if ( voter->is_proxy ) {
               enu_assert( vp.proxy == 0 , "account registered as a proxy is not allowed to use a proxy" );
            }
            ...
            if( voter->proxy ) {
               ...
               enu_assert( old_proxy != voters_tbl.end(), "old proxy not found" ); //data corruption
            ...
            if ( vp.proxy ) {
               ...
               enu_assert( new_proxy != voters_tbl.end() && new_proxy->is_proxy, "proxy not found" );
            ...
            for ( auto it = elected.begin(); it != end_it; ++it ) {
               auto prod = producers_tbl.find( *it );
               enu_assert( prod != producers_tbl.end(), "producer is not registered" );
               if ( vp.proxy == 0 ) { //direct voting, in case of proxy voting update total_votes even for inactive producers
                  enu_assert( prod->active(), "producer is not currently registered" );
               }
               producers_tbl.modify( prod, 0, [&]( auto& pi ) { pi.total_votes += votes; } );
            }
         ...
         static void on( const regproxy& reg ) {
            ...
            if ( proxy != voters_tbl.end() ) {
               enu_assert( proxy->is_proxy == 0, "account is already a proxy" );
               enu_assert( proxy->proxy == 0, "account that uses a proxy is not allowed to become a proxy" );
         ...
         static void on( const unregproxy& reg ) {
            ...
            voters_table voters_tbl( SystemAccount, SystemAccount );
            auto proxy = voters_tbl.find( reg.proxy );
            enu_assert( proxy != voters_tbl.end(), "proxy not found" );
            enu_assert( proxy->is_proxy == 1, "account is not a proxy" );
