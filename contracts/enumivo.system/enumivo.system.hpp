   ...
   class contract : public delegate_bandwidth<SystemAccount>, public native<SystemAccount> {
      public:
         ...
         static void on(const claimrewards& cr) {
            ...
            enu_assert(current_sender() == account_name(), "claimrewards can not be part of a deferred transaction");
            producers_table producers_tbl(SystemAccount, SystemAccount);
            auto prod = producers_tbl.find(cr.owner);
            enu_assert(prod != producers_tbl.end(), "account name is not in producer list");
            enu_assert(prod->active(), "producer is not active"); // QUESTION: Why do we want to prevent inactive producers from claiming their earned rewards?
            if( prod->last_rewards_claim > 0 ) {
               enu_assert(now() >= prod->last_rewards_claim + seconds_per_day, "already claimed rewards within a day");
            }
            ...
            enu_assert( rewards > system_token_type(), "no rewards available to claim" );
