...
namespace enumivosystem {
   ...
   class contract : enumivo::contract, public delegate_bandwidth<SystemAccount>, public native<SystemAccount> {
      public:
         ...
         static void on(const claimrewards& cr) {
            ...
            if (is_among_payed_producers && total_producer_votes > 0) {
               if( global_state_singleton::exists() ) {
                  ...
                  auto share_of_enu_bucket = enumivo::asset( static_cast<int64_t>( (prod->total_votes * uint64_t(parameters.enu_bucket.amount) ) / total_producer_votes ) );
            ...
            INLINE_ACTION_SENDER(enumivo::token, transfer)( N(enumivo.coin), {N(enumivo),N(active)},
                                                          { N(enumivo), cr.owner, rewards, std::string("producer claiming rewards") } );
         }
