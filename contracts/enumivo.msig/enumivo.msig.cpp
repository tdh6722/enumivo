...
void multisig::propose() {
   ...
   enumivo_assert( trx_header.expiration > now(), "transaction expired" );
...
void multisig::approve( account_name proposer, name proposal_name, permission_level level ) {
   ...
   enumivo_assert( prop_it != proptable.end(), "proposal not found" );

   auto itr = std::find( prop_it->requested_approvals.begin(), prop_it->requested_approvals.end(), level );
   enumivo_assert( itr != prop_it->requested_approvals.end(), "approval is not on the list of requested approvals" );
...
void multisig::unapprove( account_name proposer, name proposal_name, permission_level level ) {
   ...
   enumivo_assert( itr != prop_it->provided_approvals.end(), "no approval previously granted" );
...
void multisig::cancel( account_name proposer, name proposal_name, account_name canceler ) {
   ...
   if( canceler != proposer ) {
      enumivo_assert( unpack<transaction>( prop_it->packed_transaction ).expiration < now(), "cannot cancel until expiration" );
   }
