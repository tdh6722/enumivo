...
void test_action::read_action_normal() {
   ...
   enu_assert(action_data_size() == sizeof(dummy_action), "action_size() == sizeof(dummy_action)");

   total = read_action_data(buffer, 30);
   enu_assert(total == sizeof(dummy_action) , "read_action(30)" );

   total = read_action_data(buffer, 100);
   enu_assert(total == sizeof(dummy_action) , "read_action(100)" );

   total = read_action_data(buffer, 5);
   enu_assert(total == 5 , "read_action(5)" );

   total = read_action_data(buffer, sizeof(dummy_action) );
   enu_assert(total == sizeof(dummy_action), "read_action(sizeof(dummy_action))" );

   dummy_action *dummy13 = reinterpret_cast<dummy_action *>(buffer);

   enu_assert(dummy13->a == DUMMY_ACTION_DEFAULT_A, "dummy13->a == DUMMY_ACTION_DEFAULT_A");
   enu_assert(dummy13->b == DUMMY_ACTION_DEFAULT_B, "dummy13->b == DUMMY_ACTION_DEFAULT_B");
   enu_assert(dummy13->c == DUMMY_ACTION_DEFAULT_C, "dummy13->c == DUMMY_ACTION_DEFAULT_C");
}
...
void test_action::test_dummy_action() {
   ...
   enu_assert( total > 0, "get_action failed" );
   enumivo::action act = enumivo::get_action( 1, 0 );
   enu_assert( enumivo::pack_size(act) == static_cast<size_t>(total), "pack_size does not match get_action size" );
   enu_assert( act.account == N(testapi), "expected testapi account" );
   ...
   if ( dum13.b == 200 ) {
      ...
      enu_assert(false, "get_context_free_data() not allowed in non-context free action");
   } else {
      enu_assert(dum13.a == DUMMY_ACTION_DEFAULT_A, "dum13.a == DUMMY_ACTION_DEFAULT_A");
      enu_assert(dum13.b == DUMMY_ACTION_DEFAULT_B, "dum13.b == DUMMY_ACTION_DEFAULT_B");
      enu_assert(dum13.c == DUMMY_ACTION_DEFAULT_C, "dum13.c == DUMMY_ACTION_DEFAULT_C");
   }
}
...
void test_action::test_cf_action() {
   ...
   if ( cfa.payload == 100 ) {
      ...
      dum_act.send();
      enu_assert( false, "action send should not be allowed" );
   }
...
void test_action::require_notice(uint64_t receiver, uint64_t code, uint64_t action) {
   ...
   if( receiver == N(testapi) ) {
      ...
      enu_assert(false, "Should've failed");
...
void test_action::assert_false() {
   enu_assert(false, "test_action::assert_false");
}

void test_action::assert_true() {
   enu_assert(true, "test_action::assert_true");
}
...
void test_action::test_abort() {
   abort();
   enu_assert( false, "should've aborted" );
}

void test_action::test_publication_time() {
   uint32_t pub_time = 0;
   read_action_data(&pub_time, sizeof(uint32_t));
   enu_assert( pub_time == publication_time(), "pub_time == publication_time()" );
}
...
void test_action::test_current_receiver(uint64_t receiver, uint64_t code, uint64_t action) {
   ...
   enu_assert( receiver == cur_rec, "the current receiver does not match" );
}

void test_action::test_current_sender() {
   account_name cur_send;
   read_action_data(&cur_send, sizeof(account_name));
   enu_assert( current_sender() == cur_send, "the current sender does not match" );
}

void test_action::now() {
   uint32_t tmp = 0;
   uint32_t total = read_action_data(&tmp, sizeof(uint32_t));
   enu_assert( total == sizeof(uint32_t), "total == sizeof(uint32_t)");
   enu_assert( tmp == ::now(), "tmp == now()" );
}
