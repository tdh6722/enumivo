...
void test_action::test_dummy_action() {
   ...
   enumivo_assert( eosio::pack_size(act) == static_cast<size_t>(total), "pack_size does not match get_action size" );
...
void test_action::test_current_sender() {
   ...
   enumivo_assert( current_sender() == cur_send, "the current sender does not match" );
}

void test_action::now() {
   uint32_t tmp = 0;
   uint32_t total = read_action_data(&tmp, sizeof(uint32_t));
   enumivo_assert( total == sizeof(uint32_t), "total == sizeof(uint32_t)");
   enumivo_assert( tmp == ::now(), "tmp == now()" );
}
