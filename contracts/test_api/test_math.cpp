...
void test_math::test_multeq() {
  ...
  enu_assert( n == sizeof(u128_action), "test_multeq n == sizeof(u128_action)" );

  uint128_t self  = *(act.values);
  uint128_t other = *(act.values+1);
  enumivo::multeq(self, other);
  enu_assert( self == act.values[2], "test_multeq act.values[0] == act.values[2]" );
}

void test_math::test_diveq() {
  u128_action act;
  auto n = read_action_data(&act, sizeof(u128_action));
  enu_assert( n == sizeof(u128_action), "test_diveq n == sizeof(u128_action)" );

  uint128_t self  = *(act.values);
  uint128_t other = *(act.values+1);

  enumivo::diveq(self, other);
  enu_assert( self == act.values[2], "test_diveq act.values[0] == act.values[2]" );
}
...
void test_math::test_i64_to_double()
{
   ...
   enu_assert(i[0] == d, "test_i64_to_double i[0] == d");

   d = i64_to_double(uint64_t(-2));
   enu_assert(i[1] == d, "test_i64_to_double i[1] == d");

   d = i64_to_double(100000);
   enu_assert(i[2] == d, "test_i64_to_double i[2] == d");

   d = i64_to_double(uint64_t(-100000));
   enu_assert(i[3] == d, "test_i64_to_double i[3] == d");

   d = i64_to_double(0);
   enu_assert(0 == d, "test_i64_to_double 0 == d");
}
...
void test_math::test_double_to_i64()
{
   ...
   enu_assert(2 == i, "test_double_to_i64 2 == i");

   i = (int64_t)double_to_i64(d[1]);
   enu_assert(-2 == i, "test_double_to_i64 -2 == i");

   i = (int64_t)double_to_i64(d[2]);
   enu_assert(100000 == i, "test_double_to_i64 100000 == i");

   i = (int64_t)double_to_i64(d[3]);
   enu_assert(-100000 == i, "test_double_to_i64 -100000 == i");

   i = (int64_t)double_to_i64(0);
   enu_assert(0 == i, "test_double_to_i64 0 == i");
}
...
void test_math::test_double_api() {
  ...
  enu_assert( double_to_i64(res) == 1, "double funcs");
