...
void test_transaction::send_action_large() {
   ...
   act.send();
   enu_assert(false, "send_message_large() should've thrown an error");
}
...
void test_transaction::test_tapos_block_prefix() {
   ...
   enu_assert( tbp == tapos_block_prefix(), "tapos_block_prefix does not match" );
}
...
void test_transaction::test_tapos_block_num() {
   ...
   enu_assert( tbn == tapos_block_num(), "tapos_block_num does not match" );
}
...
void test_transaction::test_transaction_size() {
   ...
   enu_assert( trans_size == transaction_size(), "transaction size does not match" );
}
...
void test_transaction::send_transaction_empty(uint64_t receiver, uint64_t, uint64_t) {
   ...
   enu_assert(false, "send_transaction_empty() should've thrown an error");
}
...
void test_transaction::assert_false_error_handler(const enumivo::deferred_transaction& dtrx) {
   auto onerror_action = enumivo::get_action(1, 0);
   enu_assert( onerror_action.authorization.at(0).actor == dtrx.actions.at(0).account,
...
void test_transaction::send_transaction_large(uint64_t receiver, uint64_t, uint64_t) {
   ...
   enu_assert(false, "send_transaction_large() should've thrown an error");
}
...
void test_transaction::send_transaction_expiring_late(uint64_t receiver, uint64_t, uint64_t) {
   ...
   enu_assert(false, "send_transaction_expiring_late() should've thrown an error");
}
...
void test_transaction::send_cf_action_fail() {
   ...
   act.send_context_free();
   enu_assert(false, "send_cfa_action_fail() should've thrown an error");
}
...
void test_transaction::read_inline_action() {
   ...
   enu_assert(res == -1, "get_action error 0");
   ...
   enu_assert(res != -1, "get_action error");
   ...
   enu_assert(dres.a == 1 && dres.b == 2 && dres.c == 3, "data_as error");

   res = get_action( 3, 1, buffer, 64);
   enu_assert(res == -1, "get_action error -1");
}
