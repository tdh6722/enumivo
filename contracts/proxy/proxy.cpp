...
namespace proxy {
   ...
   namespace configs {
      ...
      bool get(config &out, const account_name &self) {
         ...
         if (it != -1) {
            ...
            enu_assert(size == sizeof(config), "Wrong record size");
            return true;
         } else {
            return false;
         }
   ...
   void apply_transfer(uint64_t receiver, account_name code, const T& transfer) {
      ...
      enu_assert(get_res, "Attempting to use unconfigured proxy");
      if (transfer.from == self) {
         enu_assert(transfer.to == code_config.owner,  "proxy may only pay its owner" );
      } else {
         enu_assert(transfer.to == self, "proxy is not involved in this transfer");
   ...
   void apply_onerror(uint64_t receiver, const deferred_transaction& failed_dtrx ) {
      ...
      const auto self = receiver;
      config code_config;
      enu_assert(configs::get(code_config, self), "Attempting use of unconfigured proxy");
