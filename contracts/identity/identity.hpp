   ...
   class contract {
      public:
         ...
         static void on( const create& c ) {
            ...
            enu_assert( itr == t.end(), "identity already exists" );
            enu_assert( c.identity != 0, "identity=0 is not allowed" );
         ...
         static void on( const certprop& cert ) {
            ...
            idents_table t( code, code );
            enu_assert( t.find( cert.identity ) != t.end(), "identity does not exist" );
            ...
            for( const auto& value : cert.values ) {
               auto idx = certs.template get_index<N(bytuple)>();
               if (value.confidence) {
                  enu_assert(value.type.size() <= 32, "certrow::type should be not longer than 32 bytes");
                  ...
                  if (value.property == N(owner)) {
                     enu_assert(sizeof(account_name) == value.data.size(), "data size doesn't match account_name size");
