...
void test_db::primary_i64_general(uint64_t receiver, uint64_t code, uint64_t action)
{
   ...
   // get
   {
      ...
      value[buffer_len] = '\0';
      std::string s(value);
      enumivo_assert(uint32_t(len) == strlen("bob's info"), "primary_i64_general - db_get_i64");
      enumivo_assert(s == "bob's", "primary_i64_general - db_get_i64");

      buffer_len = 20;
      db_get_i64(itr, value, buffer_len);
      value[buffer_len] = '\0';
      std::string sfull(value);
      enumivo_assert(sfull == "bob's info", "primary_i64_general - db_get_i64");
   }
