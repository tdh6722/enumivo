  ...
  struct token {
    ...
    */
    ...
    token( const asset& a ):quantity(NumberType(a.amount)) {
       enu_assert( a.symbol == Symbol, "attempt to construct token from asset with different symbol" );
       enu_assert( 0 <= a.amount, "attemt to convert asset with negative value to token" );
    }
    ...
    */
    token& operator-=( const token& a ) {
      enu_assert( quantity >= a.quantity, "integer underflow subtracting token balance" );
      quantity -= a.quantity;
      return *this;
    }
    ...
    */
    token& operator+=( const token& a ) {
      enu_assert( quantity + a.quantity >= a.quantity, "integer overflow adding token balance" );
      quantity += a.quantity;
      return *this;
    }
    ...
    */
    token& operator*=( uint64_t a ) {
      enu_assert( a == 0 || (quantity * a) / a == quantity, "integer overflow multiplying token balance" );
      quantity *= a;
      return *this;
    }
  ...
  struct price
  {
    ...
    */
    price( BaseToken base, QuoteToken quote ) {
      enu_assert( base  >= BaseToken(1ul), "invalid price" );
      enu_assert( quote >= QuoteToken(1ul), "invalid price" );
