#!/bin/bash
#
# enumivo-tn_roll is used to have all of the instances of the EOS daemon on a host brought down
...
prog=""
RD=""
for p in eosd enumivod nodeos; do
    prog=$p
    RD=bin
    if [ -f $RD/$prog ]; then
        break;
...
if [ \( -z "$prog" \) -o \( -z "$RD" \) ]; then
    echo unable to locate binary for eosd or enumivod or nodeos
    exit 1
fi
