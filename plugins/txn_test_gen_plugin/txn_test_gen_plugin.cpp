...
struct txn_test_gen_plugin_impl {
   void create_test_accounts(const std::string& init_name, const std::string& init_priv_key) {
      ...
      contracts::abi_def enumivo_coin_abi_def = fc::json::from_string(enumivo_coin_abi).as<contracts::abi_def>();
      ...
      //set enumivo.coin contract & initialize it
      {
...

         {
            ...
            handler.account = newaccountC;
            handler.abi = enumivo_coin_abi_def;
            trx.actions.emplace_back( vector<chain::permission_level>{{newaccountC,"active"}}, handler);
         }

         abi_serializer enumivo_coin_serializer(enumivo_coin_abi_def);
   ...
   abi_serializer enumivo_coin_serializer = fc::json::from_string(enumivo_coin_abi).as<contracts::abi_def>();
};
