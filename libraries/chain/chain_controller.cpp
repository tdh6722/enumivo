...
{
   ENU_ASSERT( auths.size() == 1, tx_irrelevant_auth,
               "updateauth action should only have one declared authorization" );
   const auto& auth = auths[0];
   ENU_ASSERT( auth.actor == update.account, tx_irrelevant_auth,
                                                                   ...
                                                                   const vector<permission_level>& auths )const
{
   ...
   const auto& auth = auths[0];
   ENU_ASSERT( auth.actor == del.account, tx_irrelevant_auth,
                                                                 ...
                                                                 const vector<permission_level>& auths )const
{
   ...
   const auto& auth = auths[0];
   ENU_ASSERT( auth.actor == link.account, tx_irrelevant_auth,
               "the owner of the linked permission needs to be the actor of the declared authorization" );

   ENU_ASSERT( link.type != contracts::updateauth::get_name(),  action_validate_exception,
               "Cannot link enumivo::updateauth to a minimum permission" );
   ENU_ASSERT( link.type != contracts::deleteauth::get_name(),  action_validate_exception,
               "Cannot link enumivo::deleteauth to a minimum permission" );
   ENU_ASSERT( link.type != contracts::linkauth::get_name(),    action_validate_exception,
               "Cannot link enumivo::linkauth to a minimum permission" );
   ENU_ASSERT( link.type != contracts::unlinkauth::get_name(),  action_validate_exception,
               "Cannot link enumivo::unlinkauth to a minimum permission" );
   ENU_ASSERT( link.type != contracts::canceldelay::get_name(), action_validate_exception,
                                                                   ...
                                                                   const vector<permission_level>& auths )const
{
   ...
   const auto& auth = auths[0];
   ENU_ASSERT( auth.actor == unlink.account, tx_irrelevant_auth,
               "the owner of the linked permission needs to be the actor of the declared authorization" );
...
const producer_object& chain_controller::validate_block_header(uint32_t skip, const signed_block& next_block)const { try {
   ...
   ENU_ASSERT( next_block.schedule_version == expected_schedule_version , block_validate_exception,"wrong producer schedule version specified ${x} expected ${y}",
