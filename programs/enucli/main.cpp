...
fc::variant call( const std::string& url,
                  ...
                  const T& v ) {
   try {
      return enumivo::client::http::call( url, path, fc::variant(v) );
   }
...
enumivo::chain_apis::read_only::get_info_results get_info() {
   return ::call(url, get_info_func, fc::variant()).as<enumivo::chain_apis::read_only::get_info_results>();
}
...
struct delegate_bandwidth_subcommand {
   ...
   delegate_bandwidth_subcommand(CLI::App* actionRoot) {
      ...
      delegate_bandwidth->add_option("stake_storage_quantity", stake_storage_amount, localized("The amount of ENU to stake for storage"))->required();
      ...
      delegate_bandwidth->set_callback([this] {
         fc::variant act_payload = fc::mutable_variant_object()
                  ...
                  ("stake_net_quantity", stake_net_amount + " ENU")
                  ("stake_cpu_quantity", stake_cpu_amount + " ENU")
                  ("stake_storage_quantity", stake_storage_amount + " ENU");
   ...
   undelegate_bandwidth_subcommand(CLI::App* actionRoot) {
      ...
      undelegate_bandwidth->set_callback([this] {
         fc::variant act_payload = fc::mutable_variant_object()
                  ("from", from_str)
                  ("receiver", receiver_str)
                  ("unstake_net_quantity", unstake_net_amount + " ENU")
                  ("unstake_cpu_quantity", unstake_cpu_amount + " ENU")
...
struct postrecovery_subcommand {
   ...
   postrecovery_subcommand(CLI::App* actionRoot) {
      ...
      post_recovery->add_option("data", json_str_or_file, localized("The authority to post the recovery with as ENU public key, JSON string, or filename"))->required();
...
int main( int argc, char** argv ) {
   ...
   app.add_option( "-H,--host", old_host_port, localized("the host where enunode is running") )->group("hidden");
   app.add_option( "-p,--port", old_host_port, localized("the port where enunode is running") )->group("hidden");
   app.add_option( "--wallet-host", old_host_port, localized("the host where enuwallet is running") )->group("hidden");
   app.add_option( "--wallet-port", old_host_port, localized("the port where enuwallet is running") )->group("hidden");
   ...
   propose_action->set_callback([&] {
      ...
      if (accountPermissions.empty()) {
         if (!proposer.empty()) {
            ...
            ENU_THROW(tx_missing_auth, "Authority is not provided (either by multisig parameter <proposer> or -p)");
         }
      }
      if (proposer.empty()) {
   ...
   cancel->set_callback([&]() {
      ...
      if (accountPermissions.empty()) {
         if (!canceler.empty()) {
            ...
            ENU_THROW(tx_missing_auth, "Authority is not provided (either by multisig parameter <canceler> or -p)");
         }
      }
      if (canceler.empty()) {
   ...
   exec->set_callback([&] {
      ...
      if (accountPermissions.empty()) {
         if (!executer.empty()) {
            ...
            ENU_THROW(tx_missing_auth, "Authority is not provided (either by multisig parameter <executer> or -p)");
         }
      }
      if (executer.empty()) {
